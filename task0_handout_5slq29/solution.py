...
def log_posterior_probs(x):
    ...
    #first get likelyhood based on data x
    ...
    log_priors = np.log(PRIOR_PROBS) #convert prior into log space
    log_sum_unnormalised = log_likelyhoods + log_priors #combine original likelyhood with prior
    log_probabilites = log_sum_unnormalised - logsumexp(log_sum_unnormalised) #normalise probabilities by 1) converting back to normal space (exp) then summing this up and then converting it back to log space 2) substracting this from original log-probability, which is equivalent to deviding in normla space as opposed to log space
    log_p = log_probabilites
